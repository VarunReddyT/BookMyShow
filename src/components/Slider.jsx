    ...
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 bg-black bg-opacity-30">
        <div className="absolute left-20 top-1/4 w-1/3">
          <h1 className="text-5xl font-bold text-white mb-4">{locations[currentIndex]?.title}</h1>
          <p className="text-white mb-6">{locations[currentIndex]?.description}</p>
          <button className="bg-blue-600 text-white px-6 py-2 rounded cursor-pointer">Explore</button>
        </div>

        <div className="absolute right-20 top-20 w-1/2 overflow-hidden">
          <div className="flex transition-transform duration-500" style={{ transform: `translateX(-${currentIndex * 30}%)` }}>
